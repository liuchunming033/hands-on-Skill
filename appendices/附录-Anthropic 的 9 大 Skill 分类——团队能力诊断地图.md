...
> 这是本课程的参考资料，用于帮助团队诊断能力缺口，不作为核心教程内容。
>
> **参考来源**：Thariq (Anthropic核心开发者) 《Lessons from Building Claude Code: How We Use Skills》
>
> **阅读对象**：已经学完本课程前 27 章，希望参考 Anthropic 内部实践来建设团队 Skill 能力的读者。
...
**课程连接**：对应第 07 章 ToolWrapper 模式和第 13 章 Gotchas 坑点。它把内部库的"使用陷阱"整理成 Agent 可主动规避的规则。
...
**课程连接**：对应第 14 章文件组织与渐进式披露。它把数据源的访问方式、常用查询语句整理成 Agent 能直接调用的东西，避免"我要分析数据，但不知道从哪里拉"。
...
**课程连接**：对应第 11 章 Pipeline 模式。它把"排障"这个非结构化能力写成症状→工具→查询模式的 Pipeline，让排障能力可复用。
...
**课程连接**：对应第 08 章 Generator 模式。它把团队的"代码风格偏好"固化成模板，Agent 只需要填充具体业务逻辑。
...
**课程连接**：对应第 16 章设置流程与内存。它用 config.json 和日志追加让 Skill 有记忆，避免每次都问相同问题。
...
**课程连接**：对应第 09 章 Reviewer 模式。它把"代码审查"从主观判断变成可执行的检查规则，并用 Adversarial Review 对抗模型自我认同偏差。
...
**课程连接**：对应第 18 章脚本——给 Agent 可调用的代码。它把"验证"封装成脚本，Agent 调用脚本即可证明自己，不需要重新实现测试逻辑。
...
**课程连接**：对应第 11 章 Pipeline 模式和第 19 章 Hooks 按需激活。它把部署写成有 Gate 的流程，并可通过 PreToolUse Hook 阻断危险操作。
...
**课程连接**：对应第 27 章 Skill 安全三原则。它把"安全意识"写成 Always / Ask First / Never，可直接迁移到团队 Skill。

---

## 与课程五大模式的映射

如果用本课程的五大设计模式来看，Anthropic 的 9 大 Skill 分类几乎覆盖了所有模式：
...
**课程连接**：对应第 13 章"不写已知知识——Agent 已经很聪明"。
...
**课程连接**：对应第 13 章"Gotchas 坑点——最有价值的内容是踩过的坑"。
...
**课程连接**：对应第 14 章"文件组织与渐进式披露——Skill 是文件夹不是文件"。
...
**课程连接**：对应第 15 章"避免过度约束——约束目标，不约束路径"。
...
**课程连接**：对应第 16 章"设置流程与内存——让 Skill 有记忆"。
...
**课程连接**：对应第 03 章"YAML Frontmatter 的精髓——写好 description 是成功的一半"。
...
**课程连接**：对应第 17 章"脚本——给 Agent 可调用的代码"。
...
**课程连接**：对应第 18 章"Hooks——给 Skill 临时规则"。
...
**课程连接**：对应第 26 章"Skill 的长期维护与团队管理"。
...
**课程连接**：对应第 24 章"六类评估指标——量化 Skill 表现"。
...
本课程前 27 章讲的是 Skill 设计方法论，而 Anthropic 的 9 大 Skill 分类提供了一个完整的生产实践样本。

| 本课程关注点 | Anthropic 9 大 Skill 分类的补充 |
|--------------|--------------------------------|
...
如果说本课程教你"如何设计 Skill"，Anthropic 的实践教你"一个生产工程团队会把哪些流程写成 Skill"。
