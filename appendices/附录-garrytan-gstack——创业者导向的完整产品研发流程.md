...
> 这是本教程的参考资料，用于帮助团队从 gstack 项目中学习创业者导向的完整产品交付流程。
>
...
**教程连接**：对应第 08 章 Generator 模式的精神内核——不是"生成模板"，而是"永远输出完整产物"。Ehthos 通过 preamble 注入，确保每个 Skill 调用的 Agent 都带着"做全"的心态开工。
...
**教程连接**：对应第 18 章 Hooks 的最大规模实践——不是"拦截工具调用"，而是"每次 Skill 启动时自动运行基础设施代码"。
...
**教程连接**：对应第 16 章"设置流程与内存——让 Skill 有记忆"的终极实现。不是 config.json，而是 schema 化的上下文预加载。
...
**教程连接**：对应第 10 章 Inversion 模式的极致实践——不是"先问需求"，而是"用 YC 合伙人面试的标准先拷问需求"。
...
**教程连接**：对应第 09 章 Reviewer 模式。gstack 的创新在于审查维度不是通用 checklist，而是一个特定思想家（Karpathy）的系统框架。
...
**教程连接**：对应第 11 章 Pipeline + 第 20 章评估的结合——把 QA 从"检查点"升级为"检查→修复→重新验证→健康分数"的闭环。
...
**教程连接**：对应第 11 章 Pipeline 模式的完整实践——从合并到 PR 创建的一条龙自动化。
...
**教程连接**：对应第 27 章 Skill 安全三原则和第 18 章 Hooks。gstack 把安全做成了级联升级模式——从基础拦截到全面锁定。

---

## 与教程五大模式的映射

| 教程模式 | gstack 中的体现 | 独特创新 |
|----------|---------------|---------|
